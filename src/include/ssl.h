...
#define MAX_TLS_HANDSHAKE_EXTENSION_LEN 1024
#define TLS_EXTENSION_SERVER_NAME            (uint16_t)0x0000
#define TLS_EXTENSION_CLIENT_CERTIFICATE_URL (uint16_t)0x0001
#define TLS_EXTENSION_ALPN                   (uint16_t)0x0010
...
typedef struct alpn {
	uint16_t len;
	alpn_body_t alpn_body;
} alpn_t;

typedef struct tls_extension_hdr {
	uint16_t type;
	uint16_t len;
} tls_extension_hdr_t;
