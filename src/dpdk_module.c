...
dpdk_get_rptr(struct mtcp_thread_context *ctxt, int ifidx, int index, uint16_t *len)
{
	...
	*len = m->pkt_len;
	...
	if ((m->ol_flags & (RTE_MBUF_F_RX_IP_CKSUM_BAD | RTE_MBUF_F_RX_L4_CKSUM_BAD)) != 0) {
...
dpdk_load_module(void)
{
	...
	if (!CONFIG.multi_process || (CONFIG.multi_process && CONFIG.multi_process_is_master)) {
		...
		for (i = 0; i < num_devices_attached; ++i) {
			...
			/* init port */
			printf("Initializing port %u/ %d... ", (unsigned) portid, num_devices_attached);
			fflush(stdout);
			ret = rte_eth_dev_configure(portid, CONFIG.num_cores, CONFIG.num_cores, &port_conf);
			if (ret < 0)
				rte_exit(EXIT_FAILURE, "Cannot configure device: err=%d, port=%u\n",
					 ret, (unsigned) portid);
			ret = rte_eth_dev_set_mtu(portid, 9000);
			if (ret != 0)
				rte_exit(EXIT_FAILURE,
						"rte_eth_dev_set_mtu:err=%d, port=%u, queueid: %d\n",
...
dpdk_dev_ioctl(struct mtcp_thread_context *ctx, int nif, int cmd, void *argp)
{
	...
	case PKT_TX_TCPIP_CSUM:
		...
		mtu = (argp)?(*(uint16_t *)argp + sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_tcp_hdr)):RTE_ETHER_MTU;
		if (m->pkt_len > mtu + m->l2_len) {
			if ((dev_info[nif].tx_offload_capa & RTE_ETH_TX_OFFLOAD_TCP_TSO) == 0)
				goto dev_ioctl_err;
			m->ol_flags = RTE_MBUF_F_TX_IPV4 | RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_TCP_SEG;
		}
		else
		...
		// uint8_t *s = (uint8_t *)&iph->saddr, *d = (uint8_t *)&iph->daddr;
		// printf("%d.%d.%d.%d:%d -> %d.%d.%d.%d:%d "
		// 		"tso: %d, mtu: %d, pkt_len: %d, l4_len: %d, tso_segsz: %d, buf_len: %d, data_len: %d, nb_segs: %d\n",
		// 		*s, *(s+1), *(s+2), *(s+3), ntohs(tcph->source),
		// 		*d, *(d+1), *(d+2), *(d+3), ntohs(tcph->dest),
		// 		(m->ol_flags & RTE_MBUF_F_TX_TCP_SEG)?1:0,
		// 		mtu, m->pkt_len, m->l4_len, m->tso_segsz, m->buf_len, m->data_len, m->nb_segs);
		break;
	case PKT_RX_IP_CSUM:
