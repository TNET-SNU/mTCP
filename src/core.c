...
#endif /* EVENT_STAT */
/*----------------------------------------------------------------------------*/
float rx_average[5] = {0};
float tx_average[5] = {0};
int cnt = 0;
#ifdef NETSTAT
static inline void
...
#define TIMEOUT 1
...
#endif /* ROUND_STAT */
#endif /* NETSTAT_TOTAL */
	float rx_total = 0, tx_total = 0;
	float rx_sum = 0, tx_sum = 0;
	...
	for (i = 0; i < CONFIG.eths_num; i++) {
		if (CONFIG.eths[i].stat_print) {
			...
			rx_total += g_nstat.rx_bytes[i];
			tx_total += g_nstat.tx_bytes[i];
		}
	}
	rx_average[cnt % 5] = rx_total;
	tx_average[cnt % 5] = tx_total;
	cnt++;
	for (i = 0; i < 5; i++) {
		rx_sum += rx_average[i];
		tx_sum += tx_average[i];
	...
	// fprintf(stderr, "[Total] flows: %6u, "
	// 		"RX: %5.2lf(Gbps), 5s avg: %5.2lf(Gbps) "
	// 		"TX: %5.2lf(Gbps), 5s avg: %5.2lf(Gbps)\n", gflow_cnt,
	// 		GBPS(rx_total), GBPS(rx_sum) / 5,
	// 		GBPS(tx_total), GBPS(tx_sum) / 5);
#ifdef ENABLELRO
